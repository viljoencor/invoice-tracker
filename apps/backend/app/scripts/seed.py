...
from app.models import Org, User, OrgMember, Client, Invoice, InvoiceItem, InvoiceSeq
...
async def run():
    async with async_session_maker() as db: # type: AsyncSession
        ...
        inv = Invoice(
            org_id=org.id, client_id=c1.id, number="INV-2025-00001",
            issue_date=date.today(), due_date=date.today()+timedelta(days=30), currency="ZAR",
            subtotal_cents=100000, tax_cents=15000, total_cents=115000, balance_cents=115000,
        ...
        db.add_all([
            InvoiceItem(invoice_id=inv.id, line_no=1, description="Consulting hours", qty=10, unit_price_cents=10000, tax_rate_bp=1500, line_total_cents=115000),
        ])
        await db.commit()
